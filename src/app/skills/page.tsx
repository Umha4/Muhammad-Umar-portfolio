        ...
        <div className=" bg-white px-6 md:px-10 lg:px-14 py-12 rounded-lg shadow-lg pb-40  overflow-x-hidden max-w-screen-2xl mx-auto">
          ...
          <div className=" mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3 lg:grid-cols-5 sm:gap-y-6 sm:gap-x-8 xl:gap-x-14 text-center">
              {skills.map((skill, index) => (
                  <div 
                      key={index} 
                      className="rounded-xl bg-sky pt-3 pb-2 sm:py-4  shadow-md flex flex-col items-center hover:shadow-xl hover:scale-105 transform transition-transform duration-300 ease-in-out"
                  >
                      {/* Skill Image */}
                      <Image
                          src={skill.image}
